...
<html>
<head>
    <title>Cetak Laporan - AutoParts</title>
    <style>
...
<body onload="window.print()">
    <div class="kop">
        <h1 style="margin:0;">DAMAR WULAN AUTOPARTS</h1>
        <p style="margin:5px 0;">Spesialis AC Mobil & Radiator | Jl. Raya Otomotif No. 123, Tangerang</p>
    </div>
...
<?php if ($jenis_laporan == 'produk'): ?>
    ...
    <table>
        <thead>
            <tr>
                <th>NO</th><th>TANGGAL</th><th>INVOICE</th><th>NAMA PRODUK</th><th>QTY</th><th>HARGA SATUAN</th><th>SUBTOTAL</th>
            </tr>
        </thead>
        <tbody>
            <?php $no = 1; while ($row = mysqli_fetch_assoc($res)): $total_semua += $row['subtotal']; ?>
                <tr>
...
<?php elseif ($jenis_laporan == 'area'): ?>
    ...
    <table>
        <thead>
            <tr>
                <th>NO</th><th>TANGGAL</th><th>INVOICE</th><th>NAMA PELANGGAN</th><th>AREA PENGIRIMAN</th><th>ONGKOS KIRIM</th>
            </tr>
        </thead>
        <tbody>
            <?php $no = 1; while ($row = mysqli_fetch_assoc($res)): $total_ongkir += $row['ongkir']; ?>
                <tr>
                    <td class="text-center"><?= $no++; ?></td>
                    <td><?= date('d/m/Y', strtotime($row['tgl_transaksi'])); ?></td>
                    <td><?= $row['id_transaksi']; ?></td>
...
<?php elseif ($jenis_laporan == 'kategori'): ?>
    ...
    <table>
        <thead>
            <tr>
                <th>NO</th><th>KATEGORI</th><th>NAMA PRODUK</th><th>TOTAL QTY TERJUAL</th><th>TOTAL PENDAPATAN</th>
            </tr>
        </thead>
        <tbody>
            <?php $no = 1; while ($row = mysqli_fetch_assoc($res)): $total_semua += $row['total_rupiah']; ?>
                <tr>
                    <td class="text-center"><?= $no++; ?></td>
